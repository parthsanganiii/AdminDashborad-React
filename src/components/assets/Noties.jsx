...
const Noties = () => {
    return (
        <div className='bg-white dark:bg-[#242731] rounded-xl p-5'>
            <h1 className='text-[#242731] dark:text-white font-bold text-[20px] mb-5'>Noties</h1>
            {data.map((item) => (
                <div className='flex gap-4 py-2 items-start'>
                    ...
                    <div className='mb-2'>
                        <h1 className='text-[#000000] dark:text-white font-medium'>{item.title}</h1>
                        <p className='text-[#72767C] dark:text-[#7C7C8D]'>{item.desc}</p>
                        <button className={`${item.btn_bg} text-[14px] font-medium text-white mt-2 py-1 px-3 rounded-md`}>{item.btn}</button>
                    </div>
                </div>
            ))}
        </div>
